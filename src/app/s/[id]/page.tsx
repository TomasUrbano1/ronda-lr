...
async function getSections(vendorId:string){
  ...
  const { data } = await supabase.from('vendor_sections').select('id,name,slug,sort').eq('vendor_id', vendorId).order('sort')
  return data ?? []
}
async function getProducts(vendorId:string){
  ...
  const { data } = await supabase.from('products').select('id,title,price,description,image_url,section_id').eq('vendor_id', vendorId).eq('is_active', true).order('title')
  return data ?? []
}
