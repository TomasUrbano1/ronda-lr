...
async function getCategory(slug:string){
  ...
  const { data } = await supabase.from('categories').select('id,name,slug').eq('slug', slug).single()
  return data
}
...
export default async function CategoryPage({ params:{slug} }:{ params:{slug:string} }){
  ...
  return (
    <main className="container py-8">
      <h1 className="text-2xl font-bold mb-2" style={{fontFamily:'Poppins'}}>{category.name}</h1>
      <p className="text-neutral-700 mb-6">Tiendas en esta categoría</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {vendors.map(v => (<VendorListItem key={v.id} v={v} />))}
      </div>
    </main>
  )
}
